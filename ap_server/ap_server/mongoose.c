...
static void ns_handle_client_udp(struct ns_connection *ls) {
  char buf[NS_UDP_RECEIVE_BUFFER_SIZE];
  ssize_t n;
  socklen_t s_len = sizeof(ls->sa);

  n = recvfrom(ls->sock, buf, sizeof(buf), 0, &ls->sa.sa, &s_len);
  if (n <= 0) {
    DBG(("%p recvfrom: %s", ls, strerror(errno)));
  } else {
    iobuf_append(&ls->recv_iobuf, buf, n);
    DBG(("%p %d bytes received", ls, n));
    ns_call(ls, NS_CLIENT_UDP_RECV, &n);
  }
}
...
time_t ns_mgr_poll(struct ns_mgr *mgr, int milli) {
  ...
  for (conn = mgr->active_connections; conn != NULL; conn = tmp_conn) {
    tmp_conn = conn->next;
    if (!(conn->flags & (NSF_LISTENING | NSF_CONNECTING | NSF_CLIENT_UDP))) {
  ...
  if (select((int) max_fd + 1, &read_set, &write_set, NULL, &tv) < 0) {
    ...
    for (conn = mgr->active_connections; conn != NULL; conn = tmp_conn) {
      ...
      if (FD_ISSET(conn->sock, &read_set)) {
        if (conn->flags & NSF_CLIENT_UDP) {
          ns_handle_client_udp(conn);
        } else if (conn->flags & NSF_LISTENING) {
...
size_t mg_client_send(struct mg_connection *conn, const void *data, int data_len) {
  struct connection *c = MG_CONN_2_CONN(conn);
  ns_send(c->ns_conn, data, data_len);
...
static void call_client_udp_handler(struct connection *conn) {
  if (conn->ns_conn->recv_iobuf.len <= 0)
  {
  ...
  conn->mg_conn.content_len = conn->ns_conn->recv_iobuf.len;
  conn->mg_conn.content = conn->ns_conn->recv_iobuf.buf;
  if (call_user(conn, MG_CLIENT_UDP_REPLY) == MG_FALSE) {
    conn->ns_conn->flags |= NSF_CLOSE_IMMEDIATELY;
  }
  iobuf_remove(&conn->ns_conn->recv_iobuf, conn->mg_conn.content_len);
  conn->mg_conn.status_code = 0;
...
struct mg_connection *mg_client_udp_connect(struct mg_server *server, const char *addr) {
  struct ns_connection *nsconn;
  struct connection *conn;
  ...
  //conn->handler = handler;
  conn->mg_conn.server_param = server->ns_mgr.user_data;
  conn->ns_conn->flags = NSF_CLIENT_UDP;
...
static void mg_ev_handler(struct ns_connection *nc, int ev, void *p) {
  ...
  switch (ev) {
    ...
    case NS_RECV:
      ...
      break;
    case NS_CLIENT_UDP_RECV:
      call_client_udp_handler(conn);
      break;
